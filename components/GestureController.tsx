...
interface GestureControllerProps {
  ...
  onHandScale?: (scale: number) => void;
}

export const GestureController: React.FC<GestureControllerProps> = ({ onModeChange, currentMode, onHandPosition, onHandScale }) => {
  ...
  useEffect(() => {
      ...
      if (videoRef.current.videoWidth > 0) { // Ensure video is ready
        ...
        if (result.landmarks && result.landmarks.length > 0) {
          ...
          detectGesture(landmarks);
            ...
            if (onHandScale) {
              onHandScale(1.0); // Reset scale when no hand detected
            }
    ...
    const detectGesture = (landmarks: any[]) => {
      ...
      // Calculate hand scale based on finger spread (for zoom)
      ...
      // Also measure distance between index tip (8) and pinky tip (20) for more stable scale
      const pinkyTip = landmarks[20];
      const indexPinkyDistance = Math.hypot(indexTip.x - pinkyTip.x, indexTip.y - pinkyTip.y);
      
      // Average the distances for more stable scale detection
      const averageFingerSpread = (thumbIndexDistance + indexPinkyDistance) / 2;
      
      // Normalize scale: typical spread is around 0.1-0.3, map to 0.5-2.0 scale range
      // When fingers spread wider (larger distance), scale increases (zoom in)
      // When fingers close (smaller distance), scale decreases (zoom out)
      const normalizedScale = Math.max(0.3, Math.min(2.5, (averageFingerSpread * 8 + 0.5)));
      
      // Smooth the scale changes to avoid jitter
      const smoothingFactor = 0.3;
      const smoothedScale = lastScaleRef.current + (normalizedScale - lastScaleRef.current) * smoothingFactor;
      lastScaleRef.current = smoothedScale;
      
      if (onHandScale) {
        onHandScale(smoothedScale);
      }
      ...
      // thumbTip already declared above for scale detection
